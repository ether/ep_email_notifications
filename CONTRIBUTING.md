# Contributing to ep_email_notifications

Thanks for helping improve `ep_email_notifications` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_email_notifications` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_email_notifications from your local clone
pnpm run plugins i --path ../ep_email_notifications

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_email_notifications/` are picked up after a server restart.
...
* Etherpad version, ep_email_notifications version, browser + OS
* Steps to reproduce
